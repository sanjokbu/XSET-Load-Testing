...
from utils.non_test_methods import open_csv_file, UserDataUC05, get_body
...
class ScenarioTaskSet(SequentialTaskSet):
    ...
    def on_start(self):
        ...
        self.user_data = UserDataUC05.get_user_data(open_csv_file(cfg.path_user_data))
        ...
        logger.info(f'User {self.username} used to cancel the ticket')
    ...
    def uc05_04_cancelTicket(self) -> None:
        ...
        if len(self.flightID_list) > 0:
            ...
            with self.client.post(
                ...
                logger.info(f'User {self.username} cancelled the ticket')
        else:
            logger.info(f'Tickets not found for the user {self.username}')
